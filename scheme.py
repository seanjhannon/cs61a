...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        eval_op = scheme_eval(expr.first, env)
        validate_procedure(eval_op)
        eval_opd = expr.rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(eval_op, eval_opd, env)
        # END PROBLEM 4
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        while not(formals is nil):
            new_f.define(formals.first, vals.first)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        eval_opd = scheme_eval(expressions.rest.first, env)
        env.define(target, eval_opd)
        return target
        # END PROBLEM 5
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        eval_opd = scheme_eval(current_list.rest.first, env)
        names = Pair(current_list.first, names)
        values = Pair(eval_opd, values)
